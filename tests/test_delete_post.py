...
@pytest.mark.asyncio
async def test_delete_post_success(credentials, linkedin_http_mock):
...
@pytest.mark.asyncio
async def test_delete_post_maps_permission_error(
    credentials,
    linkedin_http_mock,
    linkedin_responses,
):
    async def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(
            403,
            json=linkedin_responses["permission_error"],
        )
...
@pytest.mark.asyncio
async def test_delete_post_maps_network_error(credentials, linkedin_http_mock):
    async def handler(request: httpx.Request) -> httpx.Response:
